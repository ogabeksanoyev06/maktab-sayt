<template>
	<div class="transition-300 flex items-center gap-2 py-1 px-3 h-10 rounded" :class="[{ '!border-red-600 !bg-white': error }, variants[variant]]">
		<slot name="prefix" />
		<input
			v-bind="{
				id,
			...
			:id="inputId"
			ref="Input"
			class="w-full h-full text-base sm:text-sm caret-primary font-normal bg-transparent outline-none"
			:value="modelValue"
			:class="[inputClass]"
