<template>
	<div class="md:pt-[var(--header-height)] pt-[80px]">
		...
		<div class="container">
			<common-section-wrapper class="items-center text-center !gap-8 py-8 md:py-16" :title="$t('talks_title')" highlighted-title :subtitle="$t('talks_desc')" />
					...
					<div :key="isPending" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 w-full">
						...
						<template v-else>
							<template v-for="page in data?.pages" :key="page">
								<card-media v-for="(item, key) in page.results || page.data || page" :key="item.id || key" :item="item" />
							</template>
						</template>
					</div>
				</transition>
