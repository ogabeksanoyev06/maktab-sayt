<template>
	<div class="md:pt-[var(--header-height)] pt-[80px]">
		...
		<div class="container py-8 md:py-16">
			<common-section-wrapper class="mb-6 md:mb-8" content-class="justify-between" :title="$t('multi_level_scores_title')" highlighted-title :direction="'row'">
				<template #rightSide>
				...
				<div :key="isPending" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
					...
					<template v-else>
						<template v-for="page in data?.pages" :key="page">
							<card-scores-multi-level v-for="(item, key) in page.results || page.data || page" :key="item.id || key" :item="item" :search="search" />
						</template>
					</template>
				</div>
			</transition>
...
const { isPending, data, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteQuery({
	queryKey: ['multi-level', searchDebounced],
	queryFn: ({ pageParam = 0 }) => {
		return $axios
			.get('ielts/Result/MultiLevel/', {
				params: {
					limit: 8,
					offset: pageParam,
