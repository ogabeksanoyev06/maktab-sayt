<template>
	<label class="group inline-flex items-center relative select-none" :class="disabled ? 'cursor-not-allowed' : 'cursor-pointer'">
		<span
			@click="handleChange"
			...
			:class="[
				...
				{
					'after:opacity-100 after:transition-all after:duration-200 after:absolute after:left-[8px] after:top-2 after:w-2 after:h-4 after:border-l-[2.2px]  !border-blue !bg-blue after:rotate-90':
						indeterminate
				},
				{
					'!border-red bg-white': error,
					'group-hover:border-blue group-hover:bg-white': !disabled
				},
				indeterminate ? '' : modelValue ? 'after:opacity-100 after:rotate-180' : '',
				modelValue ? '!border-blue !bg-blue' : '',
				checkboxStyles
			]"
		/>
		<span class="ml-2 leading-130 text-[13px]">
			<slot name="label">
				<span class="leading-130 text-[13px] text-gray-6 font-normal" :class="[labelStyles]">
					{{ label }}
				</span>
			</slot>
		</span>
